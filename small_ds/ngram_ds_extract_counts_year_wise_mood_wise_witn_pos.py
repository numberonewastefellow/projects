...
def split_data_by_word_test_case():
    moods_dict = prepare_moods_dict("E:/download/proj/n5grm/small_ds/emotion-lexicon-master/eng/","E:/download/temp/")
...
def split_data_by_word(file_name, moods_dict, output_dir, lines, chunk_number, params):
    ...
    if len(mood_year_wise) > 0:
      path =  write_to_file_pickle(mood_year_wise, file_name, output_dir + "moods_year_wise_count", chunk_number)
      moods_output_pickle_to_excel(path, os.path.join(os.path.dirname(path),os.path.splitext(os.path.basename(path))[0]))
...
def prepare_moods_dict1(moods_input_dir):
    moods_dict = {}
    duplicates = {}
    unique_moods_dict = {}
    for f in os.listdir(moods_input_dir):
        ...
        if os.path.isfile(paath):
            ...
            with open(paath) as ff:
                mm_lsg = {}
                unq_moods = []
                for m in ff.readline().split():
                    if m not in unique_moods_dict:
                        unq_moods.append(m)
                    else:
                        mm_lsg[m] = m
                        continue

                    unique_moods_dict[m] = fn
                duplicates[fn] = mm_lsg

                moods_dict[fn] = unq_moods
                print("{} has {} words".format(fn, len(unq_moods)))
