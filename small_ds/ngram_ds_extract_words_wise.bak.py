...
def split_data_by_word(file_name, output_dir, data, chunk_number, params):
    ...
    for line in data:
        ...
        if len(the_year_wise)>0:
            write_to_file_pickle( str(the_year_wise), file_name, output_dir + "the_year_wise_count", chunk_number)
        if len(word_year_wise) > 0:
            write_to_file_pickle(str(word_year_wise), file_name, output_dir + "words_year_wise_count", chunk_number)
...
def process_file(chunk_size, file_name, input_dir, num_workers, output_dir):
    ...
    try:
        ...
        print("sa => {} finished at {}".format(file_name, datetime.utcnow()))
    except Exception as e:
        pass

#main(1,1000, 1, 'E:/download/test/1/',"E:/download/test/output","no")
